...
```{r}
# Plot monthly mean, seasonally adjusted monthly mean, STL seasonality, and STL trend
ggplot(phos_monthly_avgs_df, aes(yr_mo)) +
  scale_x_date(date_breaks = "1 year", date_minor_breaks = "6 months", date_labels = "%Y-%m") +
  geom_line(aes(y=decomp_yearly$monthly_avg, color = "Monthly mean")) +
  geom_line(aes(y=decomp_yearly$season_adjust, color = "Seasonally adjusted monthly mean"), linewidth=2) +
  geom_line(aes(y=decomp_yearly$trend, color = "STL trend"), linewidth = 2) +
  geom_line(aes(y=decomp_yearly$season_year, color = "STL seasonality")) +
  labs(x = 'Year-Month',
